...
void FileDivider(vector<string> files); //takes in the main vector of files and divides it into sub-vectors
...
bool checkDirectory(string path) //checks if a valid Direcotry has been inserted
{
...
void FilesDivider(vector<string> files) //Parses each element of the vector and populates new vectors based on the file type
{
    ...
    {
        period = files.at(pos).find(".");
        fileType = files.at(pos).substr(period, -2);
    }
